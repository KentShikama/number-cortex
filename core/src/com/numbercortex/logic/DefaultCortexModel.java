...
class DefaultCortexModel implements CortexModel {
    ...
    private CortexState buildPostPlacementCortexState() {
        ...
        if (winningValues != null) {
            ...
            state = new CortexState.CortexStateBuilder(message, currentPlayer, players, chosenNumber, coordinateNumberMap, availableNumbers).win(currentPlayer, winningAttribute, winningValues).build();
        } else {
            state = new CortexState.CortexStateBuilder(message, currentPlayer, players, chosenNumber, coordinateNumberMap, availableNumbers).build();
        }
        return state;
    }
